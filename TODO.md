...
| 2 | **UI/UX improvements** | (a) Streaming: stream LLM text output token-by-token instead of blocking behind a spinner. (b) Progress feedback: show which tool is executing during ReAct steps. (c) `cli/rendering.py` is empty — implement output formatting helpers. (d) SCAN progress bar and early abort support in plan executor. |

### Phase 2 — Adapters, safety hardening, and testing

| # | Item | Notes |
|---|------|--------|
| 3 | **Simulator adapter** | Seed with a realistic device database so sub-agent search and example flows actually work. |
| 4 | **launch_display** | Currently returns `not_implemented`. Implement facility-specific display launch. |
| 5 | **Approval: selective approve** | Add option to approve/reject individual actions in a multi-write plan. |
| 6 | **Harden execute_code sandbox** | Add `resource.setrlimit`, `safe_open()` override, `sys.meta_path` import blocker in `_wrap_code`. Strip control system env vars from subprocess environment. |
| 7 | **OS-level sandbox** | `sandbox/namespace.py` is a TODO. Add bwrap or Docker isolation for production. |
| 8 | **Integration tests** | Only unit test is `test_plan.py`. Add tests for ReAct + simulator, propose_plan flow, subagent flow. |
...
- **Phase 1 focus:** Add error handling, improve UI/UX (streaming, progress).
- **Phase 2:** Simulator seeding, sandbox hardening, OS isolation, selective approval, integration tests. Facility-specific adapters (e.g. EPICS, TANGO, DOOCS) are out of scope but supported via the `ControlSystemAdapter` ABC.
